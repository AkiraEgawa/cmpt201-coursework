...
TCP, we use INET, which is TCP

3. The client is going to send some data to the server. Where does it get this
data from? How can you tell in the code?
we read from STDIN_FILENO, which tells us that we get the data from whatever the
user types

4. How does the client program end? How can you tell that in the code?
Client program ends either when an error shows up and we perform
exit(EXIT_FAILURE), or after we send a message with 0 character (empty line). we
can tell due to the check of if read > 0
*/
